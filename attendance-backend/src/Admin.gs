 ...
 * gation) across every month at once, not just the current one. Run once
 * from the editor (select recomputeLateAndOtEveryMonthOnce_ in the toolbar
 * dropdown, click Run), read View > Logs, then remove this function again --
 * not meant to stay in the codebase (the menu only keeps the one-month
 * version, the "all months" one was removed as unused).
 */
function recomputeLateAndOtEveryMonthOnce_() {
  ...
  if (scheduleMatches.length === 0) {
    Logger.log('No "Schedule YYYY-MM" sheets found -- nothing to recompute.');
    return;
  }
  ...
  Logger.log('Done. Recomputed ' + scheduleMatches.length + ' month(s). Total: ' + totalIn + ' IN row(s), ' + totalOut + ' OUT row(s) updated. Report and Summary tabs refreshed.');
}
